...
def increment_daily_streak(student_id):
    """
    Called only when a student successfully passes a level quiz (>= 60%).
...
def generate_and_save_quiz(level_id, course_context):
    """Generates 5 questions via AI for the level and saves them to DB."""
    # This calls our new AI helper function
    mcqs = ai_model.generate_level_quiz_json(course_context)
    if not mcqs:
        return False
        
    # Delete old questions for this level to keep it fresh
...
def get_quiz_for_level(level_id, course_context):
    """Fetches questions for a level. If none exist, generates them."""
    questions = fetch_all("SELECT * FROM daily_challenges WHERE level_id = %s", (level_id,))
    if len(questions) < 3: # If somehow empty or too few, regenerate
